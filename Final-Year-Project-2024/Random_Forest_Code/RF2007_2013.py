...
startDate, endDate = '2007-01-01', '2013-12-31'
...
def LOOAnalysisTrained(modelBase, xTest, yTest, label):
    results = []
    baselineR2 = r2_score(yTest, modelBase.predict(xTest))
    baselineMSE = mean_squared_error(yTest, modelBase.predict(xTest))
    baselineMAE = mean_absolute_error(yTest, modelBase.predict(xTest))

    for feature in xTest.columns:
        xTestDrop = xTest.drop(columns=[feature])
        try:
            yPredDrop = modelBase.predict(xTestDrop)
            r2Drop = baselineR2 - r2_score(yTest, yPredDrop)
            mseDrop = mean_squared_error(yTest, yPredDrop) - baselineMSE
            maeDrop = mean_absolute_error(yTest, yPredDrop) - baselineMAE
        except Exception as e:
            r2Drop, mseDrop, maeDrop = np.nan, np.nan, np.nan
            print(f"Error dropping {feature}: {e}")

        results.append({
            "Feature": feature,
            "R² Drop": round(r2Drop, 4),
            "MSE Drop": round(mseDrop, 4),
            "MAE Drop": round(maeDrop, 4)
        })
